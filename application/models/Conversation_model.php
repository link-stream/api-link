...
class Conversation_model extends CI_Model {
    ...
    public function fetch_conversation_by_user($user_id) {
        $this->db->select('a.conversation_id, a.user_id, a.viewed, a.ref_user_id, b.last_update, c.user_name');
        $this->db->from('st_conversation_user a');
        $this->db->join('st_conversation b', 'a.conversation_id = b.id');
        $this->db->join('st_user_account c', 'a.ref_user_id = c.id');
        $this->db->where('a.user_id', $user_id);
        $this->db->order_by("b.last_update", "DESC");
        $query = $this->db->get();
    ...
    public function fetch_last_conversation_msg_by_id($id) {
        $this->db->from('st_conversation_msg');
    ...
    public function fetch_conversation_msg_by_id($id) {
        $this->db->from('st_conversation_msg');
        $this->db->where('conversation_id', $id);
        $this->db->order_by("id", "DESC");
        $query = $this->db->get();
    ...
    public function fetch_conversation_user($user_id, $coversation_id) {
        $this->db->from('st_conversation_user');
        $this->db->where('user_id', $user_id);
        $this->db->where('conversation_id', $coversation_id);
        $query = $this->db->get();
        $result = $query->row_array();
        $query->free_result();
        return $result;
    ...
    public function fetch_conversation_by_search($search) {
        $this->db->from('st_conversation');
        if (!empty($search['st_user_parent'])) {
            $this->db->where('st_user_parent', $search['st_user_parent']); //By Id
        }
        if (!empty($search['st_user_child'])) {
            $this->db->where('st_user_child', $search['st_user_child']); //By Email
        }
    ...
    public function fetch_conversation_by_id($id) {
        $this->db->from('st_conversation');
        $this->db->where('id', $id);
        $query = $this->db->get();
        $result = $query->row_array();
    ...
    public function insert_conversation_msg($data) {
        $this->db->insert('st_conversation_msg', $data);
        return $this->db->insert_id();
    }

    public function update_conversation($id, $data) {
        $this->db->where('id', $id);
        $this->db->update('st_conversation', $data);
    }

    public function insert_conversation($data) {
        $this->db->insert('st_conversation', $data);
        return $this->db->insert_id();
    }

    public function insert_conversation_user($data) {
        $this->db->insert('st_conversation_user', $data);
        return $this->db->insert_id();
    }

    public function update_conversation_user($where, $data) {
        $this->db->where($where);
        $this->db->update('st_conversation_user', $data);
    }

    //
    //

    public function fetch_conversation_detail_by_id($id) {
        $this->db->from('st_conversation_detail');
        $this->db->where('conversation_id', $id);
        $query = $this->db->get();
        $result = $query->row_array();
        $query->free_result();
...
//    }
    ...
    public function fetch_payment_method_by_user_id($user_id, $processor = 'Stripe') {
        $this->db->from('st_user_payment_method');
        $this->db->where('user_id', $user_id);
