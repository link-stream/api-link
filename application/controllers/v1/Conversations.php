...
class Conversations extends RestController {
    ...
    public function __construct() {
        parent::__construct();
        //Models
        $this->load->model(array('User_model', 'Audio_model', 'Marketing_model', 'Video_model', 'Conversation_model'));
        //Libraries
    ...
    public function index_get($id = null, $conversation_id = null) {
        if (!empty($id)) {
...
//            }
            ...
            if (empty($conversation_id)) {
                ...
                $conversations = $this->Conversation_model->fetch_conversation_by_user($id);
                if (!empty($conversations)) {
                    foreach ($conversations as $conversation) {
                        $conversation_details = $this->Conversation_model->fetch_last_conversation_msg_by_id($conversation['conversation_id']);
                        $conversation['message'] = $conversation_details['message'];
//                        $conversation['created_at'] = $conversation_details['created_at'];
//                        $conversation['msg_user_id'] = $conversation_details['user_id'];
//                        //$register_user = $this->User_model->fetch_user_store_by_id($conversation_details['user_id']);
                        $conversation['image'] = 'LS_avatar.png';
                        $final_url = $this->general_library->encode_image_url('', $this->s3_path . $this->s3_folder . '/' . $conversation['image']);
                        $conversation['data_image'] = $final_url;
                        $conversations_response[] = $conversation;
                    }
                }
                $this->response(array('status' => 'success', 'env' => ENV, 'data' => $conversations_response), RestController::HTTP_OK);
            } else {
                //CONVERSATION DETAIL
                $conversation = $this->Conversation_model->fetch_conversation_user($id, $conversation_id);
                if (!empty($conversation)) {
                    $conversations_response = $this->Conversation_model->fetch_conversation_msg_by_id($conversation_id);
                }
                $this->response(array('status' => 'success', 'env' => ENV, 'data' => $conversations_response), RestController::HTTP_OK);
            }
        } else {
    ...
    public function index_post() {
        $from_user_id = (!empty($this->input->post('from_user_id'))) ? $this->input->post('from_user_id') : '';
        $to_user_id = (!empty($this->input->post('to_user_id'))) ? $this->input->post('to_user_id') : '';
        ...
        if ((!empty($from_user_id) && !empty($to_user_id) && !empty($message))) {
            ...
            $conversation_id = $this->Conversation_model->insert_conversation($conversation_data);
            $conversation_user = [
                'user_id' => $from_user_id,
                'conversation_id' => $conversation_id,
                'viewed' => 1,
                'ref_user_id' => $to_user_id
            ];
            $this->Conversation_model->insert_conversation_user($conversation_user);
            $conversation_user = [
                'user_id' => $to_user_id,
                'conversation_id' => $conversation_id,
                'viewed' => 0,
                'ref_user_id' => $from_user_id
            ];
            $this->Conversation_model->insert_conversation_user($conversation_user);
            //
    ...
    public function index_put() {
        $conversation_id = (!empty($this->put('conversation_id'))) ? $this->put('conversation_id') : '';
        $user_id = (!empty($this->put('user_id'))) ? $this->put('user_id') : '';
        $message = (!empty($this->put('message'))) ? $this->put('message') : '';
        if ((!empty($conversation_id) && !empty($user_id) && !empty($message))) {
            $this->Conversation_model->insert_conversation_msg(['conversation_id' => $conversation_id, 'message' => $message, 'user_id' => $user_id]);
            $this->Conversation_model->update_conversation($conversation_id, ['last_update' => date('Y-m-d H:i:s')]);
            $this->Conversation_model->update_conversation_user(['ref_user_id' => $user_id, 'conversation_id' => $conversation_id], ['viewed' => 0]);
            $this->response(array('status' => 'success', 'env' => ENV, 'message' => 'Message has been updated successfully.'), RestController::HTTP_OK);
        } else {
            $this->error = 'Provide complete conversation message to add';
            $this->response(array('status' => 'false', 'env' => ENV, 'error' => $this->error), RestController::HTTP_BAD_REQUEST);
        }
    }

    public function message_viewed_put() {
        $conversation_id = (!empty($this->put('conversation_id'))) ? $this->put('conversation_id') : '';
        $user_id = (!empty($this->put('user_id'))) ? $this->put('user_id') : '';
        if ((!empty($conversation_id) && !empty($user_id))) {
            $this->Conversation_model->update_conversation_user(['user_id' => $user_id, 'conversation_id' => $conversation_id], ['viewed' => 1]);
            $this->response(array('status' => 'success', 'env' => ENV, 'message' => 'Message has been updated successfully.'), RestController::HTTP_OK);
        } else {
            $this->error = 'Provide complete conversation message to view';
            $this->response(array('status' => 'false', 'env' => ENV, 'error' => $this->error), RestController::HTTP_BAD_REQUEST);
        }
    }
